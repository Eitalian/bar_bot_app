...
Единый источник правды по методологии разработки для AI-агентов в этом репозитории (Claude Code, Codex, Cursor и т.п.). `CLAUDE.md` импортирует этот файл и сам по себе не дублирует методологию — все правки методологии вносятся **в этот файл**.

## Project

Telegram-бот для управления баром: инвентарь ингредиентов, поиск коктейлей, бар-сессии, заказы от гостей барменю. Laravel 13 + PHP 8.5 + Octane (RoadRunner) + Nutgram + PostgreSQL 18, Docker Compose.
...
### Где что лежит в `.agents/`

| Папка                 | Содержимое                                                                                                  | Когда сюда писать                                                                              |
|-----------------------|-------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------|
| `specs/`              | Долгоживущие контракты: общий дизайн бота, архитектурные правила (Conversation×Action), конвенции миграций | Новый сквозной контракт, который будет жить дольше одной фазы                                  |
| `designs/`            | Дизайн-документы конкретных фаз/фич: **что** и **зачем** делаем, какие альтернативы рассмотрены             | Перед планом: проектируем фазу. Имя: `YYYY-MM-DD-phaseN-{slug}-design.md`                       |
| `plans/`              | Пошаговые планы реализации: **как** и **в каком порядке**. Шаблон — `_template.md`                          | После design'а: разбиваем фазу на задачи и группы параллельности                                |
| `knowledge/codebase.md` | Накапливающаяся карта кодобазы: статус фаз, паттерны с примерами, известные особенности                   | Финальная задача фазы обновляет файл (см. «Политика обновления codebase.md»)                    |
| `knowledge/telegram-ui.md` | Визуальный дизайн всех Telegram-экранов: тексты сообщений, наборы кнопок, условия отображения, шаги Conversations | При любом изменении текста сообщений, кнопок или шагов Conversation — **обязательно обновить** (см. «Политика обновления telegram-ui.md») |

**Правило:** новый `.md` в `.agents/` — сначала `ls .agents/` и выбор существующей папки. Не создавать новую папку и не складывать design'ы в `specs/` или `docs/`.
...
Работа ведётся в ветках, агент создаёт PR — разработчик мержит вручную на GitHub.

**Ветки:**
- Задача: `feature/BB-{N}_{slug}` — e.g. `feature/BB-4_bar-session-flow`
- `slug` — kebab-case, до 4 слов

**PR:**
- Заголовок: `BB-{N}: описание`
- Тело: только `## Summary` с bullet points по сделанному

**Порядок работы:**
1. Создать ветку перед началом задачи
2. Коммиты на ветке: `BB-{N}: description`
...
### Политика обновления telegram-ui.md

**Файл:** `.agents/knowledge/telegram-ui.md` — визуальные скрины всех Telegram-экранов.

**Обновляет агент той задачи**, которая меняет UI: текст сообщений, набор кнопок, порядок или количество шагов Conversation, условия отображения кнопок.

**Что добавлять в план:** в секцию **Steps** финальной задачи (или задачи, напрямую меняющей UI) вставить шаг:
```
- [ ] Обновить telegram-ui.md: {описание изменённых экранов}
```

**Что обновлять:** только затронутые экраны — не переписывать весь файл. Добавлять новые разделы для новых экранов, редактировать существующие при изменении.
...
**Правило пункта 3:** файл из секции "Files", который не был создан/изменён (отброшен, объединён с другим, переименован) — не автоматический стоп, но требует явного обоснования в отчёте (почему отклонились от плана). Отсутствие без обоснования — reviewer отклоняет.
...
**Команды — дословно из секции Commands.** Когда упоминаешь запуск тестов / линта / миграций / импорта — цитируй конкретный `make` target или `docker compose exec` команду из секции **Commands** выше. Не говори «запусти тесты» в обобщённой форме — пиши `make tests-feature` или `docker compose exec app php artisan test --filter=Foo`.
...
**Без хеджирования при наличии однозначного источника.** Если ответ есть в `codebase.md`, `migration-conventions.md` или в этом `AGENTS.md` — отвечай конкретно, без оговорок «возможно, в вашем проекте принято иначе» или «обычно в Laravel так-то». Открой файл и процитируй.
