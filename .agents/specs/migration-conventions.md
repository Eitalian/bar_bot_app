...
> **Обоснование размера PK.** Размер PK обосновывается в комментарии миграции — нет «дефолтного» BIGINT по умолчанию.
> `SMALLINT`/`INTEGER` допустим для таблиц с явно ограниченной кардинальностью (пример: `bar_sessions` migration — SMALLINT = 32 767 значений = ~89 лет ежедневных сессий). `BIGINT` выбирается только когда объём действительно может его потребовать.
